  ...
  return (
    <div className="grid min-w-0 gap-5">
      ...
      <Card className="relative overflow-hidden p-5 transition-all">
        <div className="pointer-events-none absolute -right-10 -top-10 h-36 w-36 rounded-full bg-[#3B82F6]/10 blur-3xl" />

        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3.5">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-[#3B82F6]/25 bg-[#3B82F6]/10 text-[#60A5FA] shadow-inner">
              <Cpu className="h-5 w-5" />
            </div>
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="text-lg font-semibold text-white">Aceleração de GPU</h2>
                <Badge
                  tone={(settings.data?.gpuAcceleration ?? true) ? "green" : "slate"}
                  className="whitespace-nowrap px-2.5 py-0.5 text-xs font-semibold"
                >
                  {(settings.data?.gpuAcceleration ?? true) ? "Ativada" : "Desativada"}
                </Badge>
              </div>
              <p className="mt-1 text-sm leading-relaxed text-[#94A3B8]">
                Utiliza a placa de vídeo para acelerar a renderização da interface e animações. Desative caso enfrente tela preta, travamentos gráficos ou falhas com drivers.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 self-end sm:self-center shrink-0">
            <Switch
              id="gpu-acceleration-switch"
              checked={settings.data?.gpuAcceleration ?? true}
              disabled={settings.isLoading || updateSettings.isPending}
              onCheckedChange={(checked) =>
                updateSettings.mutate({ gpuAcceleration: checked })
              }
              aria-label="Alternar Aceleração de GPU"
            />
          </div>
        </div>

        <div className="mt-4 flex items-center gap-2 rounded-xl border border-white/5 bg-black/25 px-3.5 py-2 text-xs text-[#94A3B8]">
          <span className="flex h-1.5 w-1.5 shrink-0 rounded-full bg-[#60A5FA]" />
