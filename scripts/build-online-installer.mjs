...
writeFileSync(
  ...
  [
    ...
    '$subtitle.Text = "Baixando e preparando a versão mais recente do launcher."',
    ...
    "  Ui {",
    '    $status.Text = "Não foi possível instalar: $message"',
    "    $progress.Value = 100",
    ...
    "        $progress.Value = [Math]::Max(8, $eventArgs.ProgressPercentage)",
    "      }",
    "    }",
    "    $client.DownloadFileCompleted += {",
    "      param($sender, $eventArgs)",
    "      if ($eventArgs.Error) { Fail $eventArgs.Error.Message; return }",
    "      if ($eventArgs.Cancelled) { Fail 'Download cancelado.'; return }",
    "      Ui { $status.Text = 'Instalando em modo silencioso...'; $progress.Value = 96 }",
    "      try {",
    '        Start-Process -FilePath $downloadPath -ArgumentList "/S" -Wait',
    "        Ui { $status.Text = 'Instalação concluída.'; $progress.Value = 100 }",
    ...
    "    $client.DownloadFileAsync([Uri]$target.DownloadUrl, $downloadPath)",
    "  } catch {",
    "    Fail $_.Exception.Message",
    "  }",
    "})",
...
writeFileSync(
  ...
  [
    "@echo off",
    "setlocal",
    'powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "%~dp0MLUltimate-Installer.ps1"',
    "exit /b %ERRORLEVEL%",
    "",
  ].join("\r\n"),
);
